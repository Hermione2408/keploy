...
func (ps *ProxySet) handleConnection(conn net.Conn, port uint32) {
	...
	fmt.Println("in proxy server", fmt.Sprintf("%v:%v", util.ToIPAddressStr(proxyState.Dest_ip), proxyState.Dest_port))
	...
	if proxyState.Dest_port == 3306 {
		mysqlparser.CaptureMySQLMessage([]byte{}, conn, dst, ps.logger)
