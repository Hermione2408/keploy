...
func handleClientQueries(h *hooks.Hook, initialBuffer []byte, clientConn, destConn net.Conn, logger *zap.Logger) ([]*models.Mock, error) {
	...
	firstIteration := true
	var operation, responseOperationRecord string
	...
	for {
		...
		responseOperationRecord = responseOperation
		if err != nil {
	...
	go recordMySQLMessage(h, mysqlRequests, mysqlResponses, operation, responseOperationRecord)
	return nil, nil
}
