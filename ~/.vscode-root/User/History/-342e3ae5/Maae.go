...
func EncodeMock(mock *models.Mock, logger *zap.Logger) (*NetworkTrafficDoc, error) {
	...
	case models.SQL:
		requests := []spec.MysqlRequestYaml{} // Make sure to define this structure as per your requirement
		for _, v := range mock.Spec.MySqlRequests {
			req := spec.MysqlRequestYaml{
				fmt.Println(v,"REQUEST YAML")
				// Add appropriate fields as per your structure
			}
			// Implement the encoding logic for SQL request if needed
			requests = append(requests, req)
		}
		responses := []spec.MysqlResponseYaml{} // Make sure to define this structure as per your requirement
